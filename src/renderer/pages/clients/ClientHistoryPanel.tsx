// ────────────────────────────────────────────────────────
// Costerra ERP — Client History Panel
// Displays the append-only change log for a client.
...
export default function ClientHistoryPanel({ clientId }: ClientHistoryPanelProps) {
    ...
    useEffect(() => {
        async function fetch() {
            setLoading(true)
            try {
                const res = await window.api.invoke(IPC_CHANNELS.CLIENT_HISTORY, clientId)
